...
### Graph 풀이시 주의점
* bfs 탐색을 할 때마다 방문 체크를 위한 visit 배열을 선언할 경우 bfs 탐색 횟수 * r * c 만큼의 엄청난 시간 복잡도가 되기 때문에 시간 초과가 일어남
* 문제에 따라 단순 2차원 visit 배열이 아닌 특정 조건에 따라 각각 방문 체크를 하는 3차원 visit 배열 고려
...
### Floyd-Warshall
* 모든 노드 사이의 최소거리를 구하는 알고리즘으로 양방향 그래프일 때 사용
* 인접 행렬(distance[][]) 이용해서 간선 정보를 저장하고 이외의 값들은 INF 값으로 초기화 (자기자신과의 거리는 0으로 초기화)
* 3중 for문으로 (k, i, j) k를 연결점으로 i와 j 노드가 연결되어 있고, 값이 기존에 저장되어 있던 값보다 작으면 distance의 값을 바꿈
* 3중 for문을 사용하기 때문에 시간 복잡도가 상당히 큰 편으로 적절한 문제에서만 사용해야 함
...
#### 1613번 : 역사 (골드3)
  ...
  * 풀이 방식 : 
  ```
  - 두 역사사건(노드)가 연결되어 있는지, 연결되어 있다면 진출노드, 진입노드가 무엇인지 알아야하기 때문에 플로이드-와샬 알고리즘을 사용
  - 보통의 플로이드-와샬과는 다르게 노드간의 거리를 구할 필요는 없이 간선이 존재한다면 distance[i][j] 값을 -1(i 사건이 앞섬) distance[j][i] 값을 1로 저장(distance는 간선이 존재하지 않는다는 INF가 아닌 선후관계가 모호함을 뜻하는 0으로 초기화)
  - 단방향 그래프이기 떄문에 애초에 최소거리는 구할 수 없음
  ```
...
최소거리, 최소시간 등을 구해야하는 문제에 사용, Queue를 사용하기 때문에 항상 거리(시간)이 똑같은 케이스들이 먼저 나오기 때문에 조건에 부합하는 케이스를 찾아냈을 때가 항상 최소임을 확정할 수 있기 때문
